...
**Current Implementation:** The Zustand store (`hooks/use-landline-store.ts`) uses 30-second polling to refresh notification data when Landline Mode is active. This works reliably but has a delay between when notifications arrive and when the UI updates.
