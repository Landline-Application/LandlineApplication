export const TERMS_OF_USE = `TERMS OF USE

Last Updated: ${new Date().toLocaleDateString()}

1. ACCEPTANCE OF TERMS

By accessing and using the Landline application ("App"), you accept and agree to be bound by the terms and provisions of this agreement.

2. USE LICENSE

Permission is granted to temporarily use the App for personal, non-commercial purposes. This is the grant of a license, not a transfer of title.

Under this license you may not:
• Modify or copy the App materials
• Use the materials for any commercial purpose
• Attempt to decompile or reverse engineer any software contained in the App
• Remove any copyright or other proprietary notations from the materials
• Transfer the materials to another person or "mirror" the materials on any other server

3. NOTIFICATION ACCESS

The App requires access to your device notifications to provide its core functionality. By accepting these terms, you acknowledge that:
• The App will read your notifications to provide notification management features
• Notification data is processed locally on your device
• You can revoke notification access at any time through device settings

4. BACKGROUND SERVICES

The App uses background services to maintain functionality. You agree that:
• The App may run background processes for notification management
• These services are designed to minimize battery usage
• You can disable these services through the App settings

5. USER RESPONSIBILITIES

You agree to:
• Use the App in compliance with all applicable laws and regulations
• Not use the App for any unlawful purpose
• Not interfere with or disrupt the App's functionality
• Maintain the security of your device

6. DISCLAIMER

The App is provided "as is" without any representations or warranties, express or implied. We make no representations or warranties in relation to the App or the information and materials provided.

7. LIMITATIONS

In no event shall Landline or its suppliers be liable for any damages (including, without limitation, damages for loss of data or profit, or due to business interruption) arising out of the use or inability to use the App.

8. ACCURACY OF MATERIALS

The materials appearing in the App could include technical, typographical, or photographic errors. We do not warrant that any of the materials are accurate, complete, or current.

9. MODIFICATIONS

We may revise these terms of use at any time without notice. By using this App, you are agreeing to be bound by the then-current version of these terms.

10. GOVERNING LAW

These terms and conditions are governed by and construed in accordance with the laws of your jurisdiction.

11. TERMINATION

We may terminate or suspend your access to the App immediately, without prior notice or liability, for any reason whatsoever, including without limitation if you breach the Terms.

12. CONTACT INFORMATION

If you have any questions about these Terms, please contact us through the App support section.

BY USING THIS APP, YOU ACKNOWLEDGE THAT YOU HAVE READ THESE TERMS OF USE AND AGREE TO BE BOUND BY THEM.`;

export const PRIVACY_POLICY = `PRIVACY POLICY

Last Updated: ${new Date().toLocaleDateString()}

1. INTRODUCTION

Welcome to Landline ("we," "our," or "us"). We respect your privacy and are committed to protecting your personal data. This privacy policy explains how we handle your information when you use our App.

2. INFORMATION WE COLLECT

2.1 Notification Data
• The App reads notifications from your device to provide notification management features
• Notification content is processed locally on your device
• We do not transmit notification content to external servers

2.2 Usage Data
• App usage statistics (feature usage, settings preferences)
• Device information (OS version, device model)
• Performance and error logs

2.3 Automatically Collected Information
• Device identifiers
• App version information
• Crash reports and diagnostic data

3. HOW WE USE YOUR INFORMATION

We use the collected information to:
• Provide and maintain the App's core functionality
• Manage and organize your notifications
• Improve and optimize the App experience
• Troubleshoot and fix technical issues
• Analyze usage patterns to enhance features

4. DATA STORAGE AND PROCESSING

• All notification data is stored locally on your device
• We do not upload or transmit your notification content to our servers
• App preferences and settings are stored on your device
• Anonymous usage statistics may be collected for analytics purposes

5. DATA SHARING AND DISCLOSURE

We do not sell, trade, or otherwise transfer your personal information to third parties. We may share information only in the following circumstances:
• With your explicit consent
• To comply with legal obligations
• To protect our rights and safety
• In connection with a business transfer or acquisition

6. YOUR RIGHTS AND CHOICES

You have the right to:
• Access the data we collect about you
• Request deletion of your data
• Export your data for backup purposes
• Opt-out of data collection where applicable
• Revoke notification access permissions at any time
• Disable background services through App settings

6.1 Data Deletion
You can permanently delete all your data from the App at any time through the Settings screen. This will remove:
• Terms of Use acceptance records
• All captured notification logs
• Landline mode settings and preferences
• All other app data

This action is permanent and cannot be undone. We recommend exporting your data before deletion if you wish to keep a backup.

7. DATA RETENTION

• Notification data is retained only as long as necessary for the App to function
• You can clear notification history at any time through the App
• You can delete ALL your data permanently through Settings > Delete All My Data
• Uninstalling the App removes all locally stored data

8. SECURITY

We implement appropriate technical and organizational measures to protect your data:
• Local encryption for sensitive data
• Secure storage practices
• Regular security updates
• No transmission of notification content over networks

9. THIRD-PARTY SERVICES

The App may use third-party services for:
• Analytics (if applicable)
• Crash reporting
• Performance monitoring

These services have their own privacy policies. We encourage you to review them.

10. CHILDREN'S PRIVACY

The App is not intended for children under 13 years of age. We do not knowingly collect personal information from children under 13.

11. CHANGES TO THIS PRIVACY POLICY

We may update our Privacy Policy from time to time. We will notify you of any changes by:
• Updating the "Last Updated" date
• Prompting you to review changes on App update
• Posting a notice in the App

12. DO NOT TRACK SIGNALS

We do not track users across third-party websites. However, some third-party services may track your browsing activities.

13. CALIFORNIA PRIVACY RIGHTS

If you are a California resident, you have specific rights regarding your personal information under CCPA.

14. EUROPEAN USERS (GDPR)

If you are in the European Economic Area (EEA), you have certain data protection rights under GDPR.

15. CONTACT US

If you have questions about this Privacy Policy, please contact us at:
• Email: [Your contact email]
• In-App support section

16. CONSENT

By using the App, you hereby consent to our Privacy Policy and agree to its terms.

YOUR USE OF THE APP SIGNIFIES YOUR ACCEPTANCE OF THIS PRIVACY POLICY.`;
